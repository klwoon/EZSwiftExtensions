...
extension UInt {
    ...
    public static func gcd(firstNum:UInt, secondNum:UInt) -> UInt {
        let remainder = firstNum % secondNum
        if remainder != 0 {
            return gcd(firstNum: secondNum, secondNum: remainder)
        } else {
            return secondNum
        }
    }
    ...
    public static func lcm(firstNum:UInt, secondNum:UInt) -> UInt {
        return firstNum * secondNum / UInt.gcd(firstNum: firstNum, secondNum: secondNum)
    }
}
